...
// SETUP
let score, gameScene, gameOverScene, gameOver, message, base, knight, food, shrimp,
    shrimps, texture, rectangle, state, baseHit, knightAnim, run;
...
function setup() {
    ...
    const numberOfShrimps: number = 5,
        speed: number = 2,
        direction: number = 1;
    ...
    for (const i = 1; i < numberOfShrimps; i++) {
        ...
        const x = randomInt(0, app.stage.width - shrimp.width);
        const y = randomInt(0, app.stage.height - shrimp.height);
